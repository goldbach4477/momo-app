export async function POST(request: Request) {
  ...
  if (isExtract) {
    systemPrompt = `分析对话内容，提取小说设定和内容。输出严格JSON格式：
{
  "codex": [
    {"type":"character","name":"角色名","description":"外貌性格描述","details":{"role":"身份","age":"年龄"}},
    {"type":"location","name":"地点名","description":"描述"},
    {"type":"item","name":"物品名","description":"描述"},
    {"type":"lore","name":"规则/设定名","description":"世界规则或魔法体系等"},
    {"type":"faction","name":"势力名","description":"描述"}
  ],
  "outline": {
    "overall": "总体剧情概要",
    "chapters": [{"number":1,"title":"章节标题","synopsis":"本章概要"}]
  },
  "chapter": {"title":"章节标题","content":"章节正文"} 或 null
}
只输出JSON。type只能是character/location/item/lore/faction/other。提取对话中所有能找到的设定信息。`;
  } else {
